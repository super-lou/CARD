...
CARD$P.variable_en = paste0("delta{centerLF}_", Horizon)
CARD$P.unit_en = "day"
CARD$P.name_en = paste0("Average change of the center of low flows between the ", Horizon_en, " horizon and historical period")
CARD$P.description_en = "Date of the minimal 10-day mean flow value below the threshold set at the maximum of VCN10"
CARD$P.method_en = paste0("1. no temporal aggregation - 10-day centered moving average (series of VC10)
...
CARD$P.variable_fr = paste0("delta{centreBE}_", Horizon)
CARD$P.unit_fr = "jour"
CARD$P.name_fr = paste0("Changement moyen du centre des basses eaux entre l'horizon ", Horizon_fr, " et la période historique")
CARD$P.description_fr = "Date de la valeur minimale des débits moyens sur 10 jours sous le seuil fixé au maximum des VCN10"
CARD$P.method_fr = paste0("1. aucune agrégation temporelle - moyenne mobile centrée sur 10 jours (série des VC10)
...
CARD$P4.funct = list(centerLF=apply_threshold)
CARD$P4.funct_args = list("VC10", lim="upLim", where="<=", what="which.minNA", select="longest")
...
CARD$P5.funct = list("delta{centerLF}_H1"=get_deltaX,
                     "delta{centerLF}_H2"=get_deltaX,
                     "delta{centerLF}_H3"=get_deltaX)
CARD$P5.funct_args = list(list("centerLF", "date",
                               past=H0, futur=H1,
