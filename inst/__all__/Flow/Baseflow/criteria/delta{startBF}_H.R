...
CARD$P.variable_en = paste0("delta{startBF}_", Horizon)
CARD$P.unit_en = "day"
CARD$P.name_en = paste0("Average change of the start of low flows between the ", Horizon_en, " horizon and historical period")
CARD$P.description_en = paste0("Date when 10 % of the annual cumulative baseflow is reached")
CARD$P.method_en = paste0("1. annual aggregation [09-01, 08-31] - date when the baseflow (Wallingford) sum corresponds to 10 % of the total sum
...
CARD$P.variable_fr = paste0("delta{debutQB}_", Horizon)
CARD$P.unit_fr = "jour"
CARD$P.name_fr = paste0("Changement moyen du début des écoulements lents entre l'horizon ", Horizon_fr, " et la période historique")
CARD$P.description_fr = paste0("Date à laquelle 10 % du cumul annuel du débit de base sont atteints")
CARD$P.method_fr = paste0("1. agrégation annuelle [01-09, 31-08] - date à laquelle la somme du débit de base (Wallingford) correspond à 10 % de la somme totale
...
CARD$P2.funct = list("delta{startBF}_H1"=get_deltaX,
                     "delta{startBF}_H2"=get_deltaX,
                     "delta{startBF}_H3"=get_deltaX)
CARD$P2.funct_args = list(list("startBF", "date",
                               past=H0, futur=H1,
