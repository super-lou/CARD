...
CARD$P.variable_en = c("med{startLF}", "med{endLF}",
                       "med{dtLF}", "med{vLF}")
CARD$P.unit_en = c("yearday", "yearday", "day", "hm^{3}")
...
CARD$P.variable_fr = c("med{debutBE}", "med{finBE}", "med{dtBE}", "med{vBE}")
CARD$P.unit_fr = c("jour de l'année", "jour de l'année", "jour", "hm^{3}")
...
CARD$P4.funct = list(startLF=apply_threshold, endLF=apply_threshold, dtLF=apply_threshold, vLF=compute_VolDef)
CARD$P4.funct_args = list(list("VC10", lim="upLim", where="<=", what="first", select="longest"), list("VC10", lim="upLim", where="<=", what="last", select="longest"), list("VC10", lim="upLim", where="<=", what="length", select="longest"), list("VC10", select="longest", upLim="upLim"))
CARD$P4.time_step = "year"
CARD$P4.sampling_period = list(max, list("Q", na.rm=TRUE))
CARD$P4.is_date = c(TRUE, TRUE, FALSE, FALSE)
CARD$P4.NApct_lim = 3

### P5 _______________________________________________________________
CARD$P5.funct = list("med{startLF}"=circular_median, "med{endLF}"=circular_median, "med{dtLF}"=median, "med{vLF}"=median)
CARD$P5.funct_args = list(list("startLF", periodicity=365.25, na.rm=TRUE), list("endLF", periodicity=365.25, na.rm=TRUE), list("dtLF", na.rm=TRUE), list("vLF", na.rm=TRUE))
